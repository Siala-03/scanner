...
export function OrdersTable({ orders, onSelectOrder }: OrdersTableProps) {
  ...
  /** Resolve the responsible waiter: assignedWaiterId → created_by → table assignment */
  const resolveWaiter = (order: Order) => {
    ...
    return waiterId ? staff.find(s => s.id === waiterId) ?? null : null;
  };
  ...
  const HeaderCell = ({ field, children }: { field: string; children: ReactNode }) => (
    <th
      ...
      <table className="w-full">
        <thead className="bg-slate-800/50 border-b border-slate-700">
          <tr>
            ...
            <th className="px-4 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Waiter</th>
            <th className="px-4 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Items</th>
            ...
            return (
              ...
              >
                <td className="px-4 py-3 text-sm font-bold text-white">
                  #{(order as any).orderNumber || order.id.slice(0, 6)}
                </td>
