...
export function App() {
  ...
  useEffect(() => {
    ...
    fetchRestaurantPublic(currentRestaurantId)
      .then((restaurant) => {
        if (!active) return;
        setRestaurantSuspended(restaurant.is_active === false);
  ...
  // Suspension gate — block access for all non-superadmin users when the restaurant is suspended
  if (restaurantSuspended && selectedRole !== 'superadmin') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-900 p-6">
        <div className="max-w-md w-full bg-slate-800 rounded-2xl shadow-2xl p-8 text-center">
          ...
          <h1 className="text-xl font-bold text-red-400 mb-3">Subscription Suspended</h1>
          <p className="text-slate-300 leading-relaxed mb-6">
            Your subscription has been suspended. Please contact the administrator or pay your pending subscription balance to restore access.
          </p>
          <p className="text-slate-500 text-sm">
            Contact: <span className="text-slate-400 font-medium">support@servv.app</span>
          </p>
        </div>
      </div>
    );
