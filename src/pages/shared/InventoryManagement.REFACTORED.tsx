 ...
 * Handles both snake_case (from DB) and camelCase (from API)
 */
...
export function InventoryManagement({ role }: InventoryManagementProps) {
  ...
  const handleSaveRow = useCallback(async (menuItemId: string, itemName: string) => {
    ...
    console.log('💾 Saving inventory:', { menuItemId, itemName, editValues });
    ...
    try {
      console.log('📤 Sending to backend:', { menuItemId, updateData });
  ...
  const handleCreatePO = useCallback(async () => {
    ...
    try {
      ...
      try {
        console.log('📧 Notifying supplier:', supplier.email);
        // TODO: Implement supplier notification via WebSocket or HTTP
        // This would emit a socket event to notify the supplier in real-time
      } catch (err) {
    ...
    <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid md:grid-cols-3 gap-3">
                ...
                <select value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)} className="px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-slate-100 text-sm">
                  ...
                  {Array.from(new Set(inventory.map(i => i.location))).filter(Boolean).map(loc => (<option key={loc} value={loc}>{loc}</option>))}
                </select>
              </div>
            </div>
          </Card>
                    ...
                    <tr key={row.item.id} className="border-b border-slate-700/20 hover:bg-slate-700/20 transition">
                      ...
                      <td className="px-4 py-3">
                        {editingRow === row.item.id ? (
                          <input type="number" value={editValues.stock ?? row.stock} onChange={(e) => setEditValues(v => ({ ...v, stock: parseInt(e.target.value || '0', 10) }))} className="w-16 px-2 py-1 rounded bg-slate-900 border border-slate-600 text-white text-sm" />
                        ) : (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          ...
          <div className="grid gap-4">
            {purchaseOrders.map(po => (
              <Card key={po.id} className="bg-slate-800/50 border-slate-700/50">
      ...
      {activeTab === 'suppliers' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {suppliers.map(sup => (
              <Card key={sup.id} className="bg-slate-800/50 border-slate-700/50">
                <div className="flex items-start justify-between mb-3">
                  <div>
                ...
                {isManager && (
                  <div className="flex gap-2">
                    <Button onClick={() => { setEditingSupplier(sup); setSupplierForm(sup); setShowSupplierModal(true); }} variant="outline" size="sm">Edit</Button>
                    <Button onClick={async () => { try { await apiUpdateSupplier(sup.id, { isActive: !sup.isActive }); await refresh(); alert(`✅ Supplier ${sup.isActive ? 'deactivated' : 'activated'}`); } catch (err) { alert(`❌ Failed: ${err instanceof Error ? err.message : 'Unknown error'}`); } }} size="sm" className={sup.isActive ? 'bg-red-500/20 text-red-400 hover:bg-red-500/30' : 'bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30'}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          ...
          <div className="grid gap-4">
            {waste.slice(0, 10).map(w => (
              <Card key={w.id} className="bg-slate-800/50 border-slate-700/50">
          ...
          <div className="space-y-2">
            ...
            {newPOItems.map((item, idx) => (
              <div key={idx} className="grid grid-cols-3 gap-2">
                ...
                <input type="number" placeholder="Qty" value={item.orderedQty} onChange={(e) => setNewPOItems(v => v.map((i, j) => j === idx ? { ...i, orderedQty: parseInt(e.target.value || '0', 10) } : i))} className="px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white text-sm" />
                <input type="number" placeholder="Cost" value={item.unitCost} onChange={(e) => setNewPOItems(v => v.map((i, j) => j === idx ? { ...i, unitCost: parseFloat(e.target.value || '0') } : i))} className="px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white text-sm" />
              </div>
            ))}
          </div>
      ...
      <Modal isOpen={showWasteModal} onClose={() => setShowWasteModal(false)} title="Record Waste">
        <div className="space-y-3">
          ...
          <input type="number" placeholder="Quantity" value={wasteForm.qty} onChange={(e) => setWasteForm(v => ({ ...v, qty: parseInt(e.target.value || '0', 10) }))} className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white" />
